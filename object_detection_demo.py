...
sys.path.append('./common/python/model_zoo')
...
def build_argparser():
    ...
    args.add_argument('-m', '--model', default='./FP32/yolox-tiny.xml',
                      help='Path to an .xml file with a trained model '
                           'or address of model inference service if using ovms adapter.')
    available_model_wrappers = [name.lower() for name in DetectionModel.available_wrappers()]
    args.add_argument('-at', '--architecture_type', default='yolox', help='Specify model\' architecture type.',
                      type=str, choices=available_model_wrappers)
    args.add_argument('--adapter', help='Specify the model adapter. Default is openvino.',
                      default='openvino', type=str, choices=('openvino', 'ovms'))
    args.add_argument('-i', '--input', default='0',
                      help='An input to process. The input must be a single image, '
                           'a folder of images, video file or camera id.')
    args.add_argument('-d', '--device', default='CPU', type=str,
                      help='Specify the target device to infer on; CPU or GPU is '
                           'acceptable. The demo will look for a suitable plugin for device specified. ')
    common_model_args = parser.add_argument_group('Common model options')
    common_model_args.add_argument('--labels', default='./label.txt', help='Labels mapping file.', type=str)
    common_model_args.add_argument('-t', '--prob_threshold', default=0.3, type=float,
...
def draw_detections(frame, detections, palette, labels, output_transform):
    ...
    for detection in detections:
        ...
        cv2.putText(frame, f'({center_x}, {center_y})',
                    (center_x + 12, center_y - 12), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 1)
